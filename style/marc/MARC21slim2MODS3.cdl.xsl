...
-->
	...
	<xsl:template name="marcRecord" match="marc:record" mode="m2m">
           <mods xtf:meta="true" xtf:store="true" xtf:index="false" xmlns:xtf="http://cdlib.org/xtf">
			...
			<xsl:for-each select="marc:subfield[@code='b']">
				<classification authority="lcc">
					<xsl:if test="../marc:subfield[@code='3']">
						<xsl:attribute name="displayLabel">
<xsl:value-of select="replace(normalize-space(../marc:subfield[@code='3']),'&amp;', '﹠')"/>
						</xsl:attribute>
					</xsl:if>
...
<xsl:value-of select="replace(normalize-space(../marc:subfield[@code='3']),'&amp;', '﹠')"/>
		...
		<xsl:for-each select="marc:datafield[@tag=024][@ind1=1]">
			<identifier type="upc">
                ...
                <xsl:apply-templates 
              select="if (marc:datafield[@tag=852]) then marc:datafield[@tag=852]
                     else if (marc:datafield[@tag=099]) then marc:datafield[@tag=099]
                     else if (marc:datafield[@tag=090]) then marc:datafield[@tag=090]
                     else if (marc:datafield[@tag=092]) then marc:datafield[@tag=092]
                     else if (marc:datafield[@tag=096]) then marc:datafield[@tag=096]
                     else marc:datafield[@tag=098]
              " mode="m852"/>
...
<!-- CDL / add call numbers to MODS -->


        <xsl:template match="marc:datafield[@tag=852]" mode="m852">
         <identifier type="852">
        ...
        <xsl:template match="marc:datafield" mode="m852">
         <identifier type="{@tag}">
          <xsl:apply-templates/>
         </identifier>
        </xsl:template>
